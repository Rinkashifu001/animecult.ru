...
Commontator.configure do |config|
  ...
  config.user_name_proc = ->(user) { user.show_title }
  ...
  config.user_mentions_proc = ->(current_user, thread, query) do
    query_fixed = russian_lower(remove_sql_explot(query)).downcase
    current_user.class.where("lower(nickname) like '#{query}%'")
  end
  ...
  # 20 + 20*5 + 20*5*2 == 320
  # Default: [ 20, 5, 2 ]
  config.comments_per_page = [ 5, 5, 2 ]
